...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Documents/portfolio-Mayande-Mamba/data/imdb_5000_messy.csv")
```
...
- we need to remove the Nas from the data set
...
-   What's goofy about this / what do we need to fix? - tebobervations are unmeasurable or not easy to interpret 
-   More specifically, what different categories does the `color` variable take, and how many movies fall into each of these categories? = COLOR, color, Color, Black and White 
...
imdb_temp <- imdb_messy |>
  mutate(color = str_replace(color, "COLOR", "Color"), color = str_replace(color, "color", "Color"), 
...
#imdb_messy |> 
 # colSums(select(is.na(actor_1_facebook_likes)))
```
