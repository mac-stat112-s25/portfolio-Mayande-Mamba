...
#Missing values can skew the data analysis and visukaization forming outliers or errors caused by missing values. 
...
penguins |>
  select(-year, -island, -body_mass_g, -sex)

penguins |>
  select(year, ends_with("mm"))
...
month(today) #Just gives the numbe rof the month 
month(today, label = TRUE) #labels the month and its degree 
```

```{r}
# What does this number mean?
week(today) #The week in which this day is on for the year 
```

```{r}
# What do these lines produce / what's their difference?
mday(today)  #Day of the month
yday(today)  # This is often called the "Julian day" # Day in the year
```

```{r}
# What do these lines produce / what's their difference?
wday(today) #Day of the week 
wday(today, label = TRUE) #lables where the day is in the week 
```
